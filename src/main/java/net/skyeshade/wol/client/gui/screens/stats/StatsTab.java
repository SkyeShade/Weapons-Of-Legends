...
@OnlyIn(Dist.CLIENT)
public class StatsTab extends GuiComponent {
   private final Minecraft minecraft;
   private final StatsScreen screen;
   private final StatsTabType type;
   ...
   private final Map<Advancement, StatsWidget> widgets = Maps.newLinkedHashMap();
   ...
   public StatsTab(Minecraft pMinecraft, StatsScreen pScreen, StatsTabType pType, int pIndex, Advancement pAdvancement, DisplayInfo pDisplay) {
      ...
      this.root = new StatsWidget(this, pMinecraft, pAdvancement, pDisplay);
      this.addWidget(this.root, pAdvancement);
   }

   public StatsTab(Minecraft mc, StatsScreen screen, StatsTabType type, int index, int page, Advancement adv, DisplayInfo info) {
   ...
   public void drawTooltips(PoseStack pPoseStack, int pMouseX, int pMouseY, int pWidth, int pHeight) {
      ...
      if (pMouseX > 0 && pMouseX < 234 && pMouseY > 0 && pMouseY < 113) {
         for(StatsWidget advancementwidget : this.widgets.values()) {
   ...
   @Nullable
   public static StatsTab create(Minecraft pMinecraft, StatsScreen pScreen, int pTabIndex, Advancement pAdvancement) {
      if (pAdvancement.getDisplay() == null) {
         return null;
      } else {
         for(StatsTabType advancementtabtype : StatsTabType.values()) {
            if ((pTabIndex % StatsTabType.MAX_TABS) < advancementtabtype.getMax()) {
               return new StatsTab(pMinecraft, pScreen, advancementtabtype, pTabIndex % StatsTabType.MAX_TABS, pTabIndex / StatsTabType.MAX_TABS, pAdvancement, pAdvancement.getDisplay());
            }
   ...
   public void scroll(double pDragX, double pDragY) {
      ...
      System.out.println(this.scrollX);
   }

   public void addAdvancement(Advancement pAdvancement) {
      if (pAdvancement.getDisplay() != null) {
         StatsWidget advancementwidget = new StatsWidget(this, this.minecraft, pAdvancement, pAdvancement.getDisplay());
         this.addWidget(advancementwidget, pAdvancement);
      }
   }

   private void addWidget(StatsWidget pWidget, Advancement pAdvancement) {
   ...
   @Nullable
   public StatsWidget getWidget(Advancement pAdvancement) {
      return this.widgets.get(pAdvancement);
   }

   public StatsScreen getScreen() {
      return this.screen;
   }
}
